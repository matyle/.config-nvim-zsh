...
function isRelationField(currentLine, lines) {
    const relationNames = (0, completions_1.getAllRelationNames)(lines);
...
function renameModelOrEnumWhereUsedAsType(block, lines, document, position, blockType) {
    ...
    const allRelationNames = (0, completions_1.getAllRelationNames)(lines);
...
function renameReferencesForFieldValue(currentValue, newName, document, lines, block, isRelationFieldRename) {
    ...
    for (const [key, item] of lines.entries()) {
        ...
        if (item.includes(relationAttribute) &&
            ...
            const fieldValues = (0, completions_1.getValuesInsideSquareBrackets)(fields);
        ...
        if (searchStringsSameBlock.some((s) => item.includes(s)) &&
            item.includes(currentValue)) {
            const currentLineUntrimmed = (0, util_1.getCurrentLine)(document, key);
            const valuesInsideBracket = (0, completions_1.getValuesInsideSquareBrackets)(currentLineUntrimmed);
    ...
    for (const [index, value] of lines.entries()) {
        if (value.includes(block.name) &&
            ...
            const referenceValues = (0, completions_1.getValuesInsideSquareBrackets)(references);
    ...
    for (const [index, value] of lines.entries()) {
        ...
        if (value.includes(currentName) &&
            ...
            if (block && block.type == 'model') {
                ...
                const types = (0, completions_1.getTypesFromCurrentBlock)(lines, block);
